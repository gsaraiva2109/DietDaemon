...
// Pure helper for onExtracted (OCR prefill): computes the next form state
// from a scan result without touching React state directly, keeping the
// branching out of the component body.
function applyExtractedDraft(
  draft: NutritionLabelDraft,
  current: FormValues,
): { values: FormValues; unreadable: boolean; lowConfidence: Set<keyof FormValues> } {
  if (draft.unreadable) {
    return { values: current, unreadable: true, lowConfidence: new Set() }
  }
  const next = { ...current }
  ...
  const lowConfidence = new Set(
    draft.low_confidence_fields.map((f) => DRAFT_FIELD_MAP[f]).filter((f): f is keyof FormValues => Boolean(f)),
  )
  return { values: next, unreadable: false, lowConfidence }
}

// Pure helper for submit: builds the mutation payload and validates it.
function buildCustomFoodInput(values: FormValues): { input: CustomFoodInput; invalid: boolean } {
  const input = {
...
// Pure helper for the save button's disabled state.
function isFormComplete(values: FormValues): boolean {
  if (!values.name.trim() || Number(values.basis_grams) <= 0) return false
...
export function CustomFoodModal({ food, onClose, onSaved }: Readonly<{
  ...
  function onExtracted(draft: NutritionLabelDraft) {
    const result = applyExtractedDraft(draft, values)
  ...
  function submit(e: SyntheticEvent) {
    e.preventDefault()
    const { input, invalid } = buildCustomFoodInput(values)
    if (invalid) return
  ...
  let saveLabel = t('customFood.save')
  if (saving) saveLabel = t('customFood.saving')
  else if (food) saveLabel = t('customFood.saveChanges')

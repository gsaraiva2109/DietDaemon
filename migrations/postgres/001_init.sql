-- 001_init: DietDaemon schema (Postgres). Greenfield rewrite — no real users/
...
-- (kept consistent across dialects even though Postgres never had this issue).
...
CREATE TABLE IF NOT EXISTS foods (
    ...
    serving_size  DOUBLE PRECISION NOT NULL DEFAULT 0,
...
-- little-endian float32 BYTEA; dim records the vector length for sanity
-- checks on load.
...
-- food_search: full-text index over the global food catalog plus each user's
-- personal aliases. Uses tsvector column + GIN index instead of FTS5
-- (SQLite-only). Rows come in two kinds:
...
BEGIN
    ...
    VALUES (NEW.food_id, '', NEW.name, '', to_tsvector('simple', coalesce(NEW.name, '')));
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS foods_fts_insert ON foods;
CREATE TRIGGER foods_fts_insert AFTER INSERT ON foods
    FOR EACH ROW EXECUTE FUNCTION foods_fts_insert_fn();
...
DROP TRIGGER IF EXISTS foods_fts_update ON foods;
CREATE TRIGGER foods_fts_update AFTER UPDATE ON foods
    FOR EACH ROW EXECUTE FUNCTION foods_fts_update_fn();

CREATE OR REPLACE FUNCTION foods_fts_delete_fn() RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS foods_fts_delete ON foods;
CREATE TRIGGER foods_fts_delete AFTER DELETE ON foods
    FOR EACH ROW EXECUTE FUNCTION foods_fts_delete_fn();
...
BEGIN
    ...
    SELECT f.food_id, NEW.user_id, f.name, NEW.alias_normalized,
           to_tsvector('simple', f.name || ' ' || coalesce(NEW.alias_normalized, ''))
    FROM foods f
...
CREATE TABLE IF NOT EXISTS pending_aliases (
    ...
    food_id     TEXT NOT NULL REFERENCES foods(food_id) ON DELETE CASCADE,
    match_score DOUBLE PRECISION NOT NULL,      -- embedding similarity score
    created_at  TEXT NOT NULL DEFAULT (NOW())
);
...
CREATE TABLE IF NOT EXISTS water_logs (
    ...
    logged_date TEXT GENERATED ALWAYS AS (substring(logged_at from 1 for 10)) STORED,
    note        TEXT,
    created_at  TEXT NOT NULL DEFAULT (NOW())
);
...
-- for exercise ordering (kept consistent across dialects).
...
CREATE TABLE IF NOT EXISTS measurement_log (
    ...
    hips_cm        DOUBLE PRECISION NOT NULL DEFAULT 0,
    chest_cm       DOUBLE PRECISION NOT NULL DEFAULT 0,
    left_arm_cm    DOUBLE PRECISION NOT NULL DEFAULT 0,
    right_arm_cm   DOUBLE PRECISION NOT NULL DEFAULT 0,
    left_thigh_cm  DOUBLE PRECISION NOT NULL DEFAULT 0,
    right_thigh_cm DOUBLE PRECISION NOT NULL DEFAULT 0,
    note           TEXT NOT NULL DEFAULT '',
    created_at     TEXT NOT NULL DEFAULT (NOW()),
    UNIQUE (user_id, date)
);
...
CREATE TABLE IF NOT EXISTS sent_nudges (
    ...
    sent_at          TEXT NOT NULL,
    body             TEXT NOT NULL,
    snapshot_kcal    DOUBLE PRECISION NOT NULL DEFAULT 0,
    snapshot_protein DOUBLE PRECISION NOT NULL DEFAULT 0,
    snapshot_carbs   DOUBLE PRECISION NOT NULL DEFAULT 0,
    snapshot_fat     DOUBLE PRECISION NOT NULL DEFAULT 0,
    snapshot_fiber   DOUBLE PRECISION NOT NULL DEFAULT 0,

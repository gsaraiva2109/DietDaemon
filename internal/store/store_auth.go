...
func (s *Store) GetPasswordHash(ctx context.Context, userID string) (string, error) {
	...
	var hash string
	if err := row.Scan(&hash); errors.Is(err, sql.ErrNoRows) {
...
func (s *Store) GetUserByAPIKey(ctx context.Context, hashedKey string) (types.User, error) {
	...
	var userID string
	if err := tx.QueryRowContext(ctx, keyQ, hashedKey).Scan(&userID); errors.Is(err, sql.ErrNoRows) {
		return types.User{}, types.ErrNotFound
	} else if err != nil {
...
// DeleteSession #nosec G202 — id is generated by the app (SHA-256 hex), not user input
...
func scanSession(row *sql.Row) (auth.Session, error) {
	...
	var remember int
	if err := row.Scan(&s.ID, &s.UserID, &s.CSRFToken, &ca, &lsa, &iea, &aea, &remember, &ip, &ua); errors.Is(err, sql.ErrNoRows) {
...
func (s *Store) GetUserByShareToken(ctx context.Context, hashedToken string) (types.User, error) {
	...
	var userID string
	if err := tx.QueryRowContext(ctx, tokenQ, hashedToken).Scan(&userID); errors.Is(err, sql.ErrNoRows) {
		return types.User{}, types.ErrNotFound
	} else if err != nil {
...
func (s *Store) GetTOTPSecret(ctx context.Context, userID string) (secret string, confirmed bool, err error) {
	...
	if err := row.Scan(&secret, &ca); errors.Is(err, sql.ErrNoRows) {
...
func (s *Store) GetMFAChallenge(ctx context.Context, id string) (userID string, remember bool, expiresAt string, err error) {
	...
	row := s.db.QueryRowContext(ctx, q, id)
	var payload string
	if err := row.Scan(&userID, &payload, &expiresAt); errors.Is(err, sql.ErrNoRows) {
		return "", false, "", types.ErrNotFound
	} else if err != nil {
...
func (s *Store) ConsumeOIDCState(ctx context.Context, id string) (nonce, pkceVerifier, linkUserID, next string, err error) {
	...
	if scanErr := row.Scan(&nonce, &pkceVerifier, &luID, &n, &expiresAt); errors.Is(scanErr, sql.ErrNoRows) {
		return "", "", "", "", types.ErrNotFound
	} else if scanErr != nil {
...
func (s *Store) ConsumeEmailToken(ctx context.Context, id, purpose string) (userID string, err error) {
	...
	row := tx.QueryRowContext(ctx, q, id)
	if scanErr := row.Scan(&userID, &storedPurpose, &expiresAt); errors.Is(scanErr, sql.ErrNoRows) {
		return "", types.ErrNotFound
	} else if scanErr != nil {
...
func (s *Store) GetMagicCode(ctx context.Context, userID string) (codeHash, expiresAt string, attempts int, err error) {
	...
	row := s.db.QueryRowContext(ctx, q, userID)
	if scanErr := row.Scan(&codeHash, &expiresAt, &attempts); errors.Is(scanErr, sql.ErrNoRows) {
...
func (s *Store) ConsumeWebAuthnSession(ctx context.Context, id string) (userID, sessionDataJSON string, err error) {
	...
	row := tx.QueryRowContext(ctx, q, id)
	var expiresAt string
	if scanErr := row.Scan(&uid, &sessionDataJSON, &expiresAt); errors.Is(scanErr, sql.ErrNoRows) {
		return "", "", types.ErrNotFound
	} else if scanErr != nil {

...
func TestToWireMessagesToolRoundTrip(t *testing.T) {
	...
	assertAssistantToolUseMessage(t, wire[1])
	assertToolResultMessage(t, wire[2])
}

func assertAssistantToolUseMessage(t *testing.T, m chatMessage) {
	t.Helper()
	if m.Role != "assistant" {
		t.Fatalf("wire[1].Role = %q, want assistant", m.Role)
	}
	if len(m.Content) != 1 || m.Content[0].Type != "tool_use" {
		t.Fatalf("wire[1].Content = %+v, want single tool_use block", m.Content)
	}
	if m.Content[0].ID != "tc_1" || m.Content[0].Name != "suggest" {
		t.Errorf("tool_use block = %+v, want ID tc_1 Name suggest", m.Content[0])
	}
	var input struct {
		Args string `json:"args"`
	}
	if err := json.Unmarshal(m.Content[0].Input, &input); err != nil || input.Args != "breakfast" {
		t.Errorf("tool_use input = %s, want {\"args\":\"breakfast\"}", m.Content[0].Input)
	}
}

func assertToolResultMessage(t *testing.T, m chatMessage) {
	t.Helper()
	if m.Role != "user" {
		t.Fatalf("wire[2].Role = %q, want user (tool_result must be a user turn)", m.Role)
	}
	if len(m.Content) != 1 || m.Content[0].Type != "tool_result" {
		t.Fatalf("wire[2].Content = %+v, want single tool_result block", m.Content)
	}
	if m.Content[0].ToolUseID != "tc_1" {
		t.Errorf("tool_result.tool_use_id = %q, want tc_1 (must match the preceding tool_use block)", m.Content[0].ToolUseID)
	}
	if m.Content[0].Content != "eat oatmeal" {
		t.Errorf("tool_result.content = %q, want %q", m.Content[0].Content, "eat oatmeal")
	}
}

...
func (a *Adapter) ExtractPlan(ctx context.Context, image []byte, mimeType string) (types.PlanDraft, error) {
	...
	payload, err := json.Marshal(body)
	if err != nil {
		return types.PlanDraft{}, fmt.Errorf("ollama: marshal vision request: %w", err)
	}
	...
	resp, err := a.client.Do(req)
	if err != nil {
		return types.PlanDraft{}, fmt.Errorf("ollama: vision generate: %w", err)
	}
	defer func() { _ = resp.Body.Close() }()

	if resp.StatusCode != http.StatusOK {
		return types.PlanDraft{}, fmt.Errorf("ollama: vision generate status %d", resp.StatusCode)
	}

	var gr generateResponse
	if err := json.NewDecoder(resp.Body).Decode(&gr); err != nil {
		return types.PlanDraft{}, fmt.Errorf("ollama: decode vision generate: %w", err)
	}
